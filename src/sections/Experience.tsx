...
function Experience() {
    return <section id="experience" className="py-32 overflow-hidden">
        <div className="container mx-auto px-6 relative z-10">
                ...
                <span className="text-cor-secundaria-texto text-sm font-medium tracking-wider uppercase">Jornada Profissional</span>
                <h2 className="text-4xl md:text-5xl font-bold mt-4 mb-6 leading-tight fade-in delay-100 text-cor-secundaria-texto">Experiência que <span className="font-serif italic font-normal text-white">fala por si.</span></h2>
            ...
            <div className="relative">
                <div className="linha-do-tempo absolute left-0 md:left-1/2 top-0 bottom-0 w-[2px] bg-gradient-to-b from-cor-primaria/70 via-cor-primaria/30 to-transparent md:-translate-x-1/2 shadow-[0_0_25px_rgba(32,178,166,0.8)]"/>
                    ...
                    {experiencias.map((exp, i) => (
                        <div key={i} className="relative grid md:grid-cols-2 gap-8 fade-in" style={{
                            ...
                            <div className={`pl-8 md:pl-0 ${i % 2 === 0 ? "md:pr-16 md:text-right" : "md:col-start-2 md:pl-16"}`}>
                                <div className={`vidro p-6 rounded-2xl border border-cor-primaria/30 hover:border-cor-primaria/50 transition-all duration-500`}>
                                    ...
                                    <div className={`flex flex-wrap gap-2 mt-4 ${i % 2 === 0 ? "md:justify-end" : ""}`}>{exp.tecnologias.map((tech, i) => (
                                        <span key={i} className="px-3 py-1 bg-cor-superficie text-xs rounded-full text-cor-neutro-texto">{tech}</span>
                                    ))}</div>
                                </div>
                            </div>
                        </div>
