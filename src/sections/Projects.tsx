...
function Projects() {
    return <section id="projects" className="py-32 relative overflow-hidden">
        <div className="container mx-auto px-6 relative z-10">
                ...
                <span className="text-cor-secundaria-texto text-sm font-medium -tracking-wider uppercase fade-in">Trabalhos em destaque</span>
                <h2 className="text-4xl md:text-5xl font-bold mt-4 mb-6 fade-in delay-100 text-cor-secundaria-texto">Projetos que <span className="font-serif italic font-normal text-white">causam impacto.</span></h2>
                ...
                {projetos.map((projeto, i) => (
                    <div key={i} className="group vidro rounded-2xl overflow-hidden fade-in md:row-span-1" style={{
                        animationDelay: `${(i + 1) * 100}ms`
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-cor-superficie-card via-cor-superficie-card/50 to-transparent opacity-60" />
                        </div>
                        <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <a href={projeto.link} className="p-3 rounded-full vidro hover:bg-cor-primaria hover:text-cor-primaria-texto transition-all">
                                <ArrowUpRight className="w-5 h-5" />
                            </a>
                            <a href={projeto.github} className="p-3 rounded-full vidro hover:bg-cor-primaria hover:text-cor-primaria-texto transition-all">
                        ...
                        <div className="p-6 space-y-4 relative z-10">
                            <div className="flex items-start justify-between">
                                <h3 className="text-xl font-semibold group-hover:text-cor-primaria transition-colors">{projeto.titulo}</h3>
                            ...
                            <div className="flex flex-wrap gap-2">{projeto.tags.map((tag, i) => (
                                <span key={i} className="px-4 py-1 rounded-full bg-cor-superficie text-xs font-medium border border-cor-borda/50 text-cor-neutro-texto hover:border-cor-primaria/50 hover:text-cor-primaria transition-all duration-300">{tag}</span>
                            ))}</div>
                        </div>
                    </div>
